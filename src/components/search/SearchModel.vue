<template>
  <q-input
    ...
    :shandout="standout"
    :rounded="rounded"
    :label="searchLabel === 'Search ' ? searchLabel + label : searchLabel"
    :placeholder="placeholder ? placeholder : null"
    input-class="text-left"
    :input-style="darkmode ? '' : 'color: black'"
    :bg-color="standout ? 'grey-3' : ''"
    :dark="!standout && darkmode"
    :standout="standout"
    class="relative"
    :style="darkmode && !standout ? 'background: #202a46' : ''"
    :color="darkmode ? 'blue-5' : 'primary'"
    :disable="disableInput"
    :rules="rules"
    :lazy-rules="rules && true"
    hide-bottom-space
    @click.once="fetchRows"
    @input="
      (val) => {
        state.search = val;
      }
    ...
    @blur="
      () => {
        closeCustomMenu();
        focusTracker(label, 'close');
      }
    "
    @keydown.tab="(event) => focusMenu(event)"
  ...
  >
    ...
    <q-menu
      v-if="state.isMenuOpenTemplate"
      ref="menuRef"
      no-focus
      :model-value="state.menuOpen"
      @hide="
        (evt) => {
          hideMenuEvent(evt, state.menuOpen);
        }
      "
      @keydown="updateKeyPressed"
    >
      <q-list
        ...
        :class="darkmode ? 'bg-[#354161] text-grey-3' : ''"
      >
        <q-scroll-area
          ref="customScrollbar"
          style="height: 20rem"
...
export default defineComponent({
  ...
  setup(props) {
    ...
    function updateKeyPressed(event) {
    ...
    onMounted(() => {
      ...
      if (!props.noFocus) {
        ...
        // if inputRef.value is focused on
      }
    });
    ...
    function setComponentID(item) {
      ...
      console.log(searchOptionsX, "searchOptionsX");
  ...
  methods: {
    focusMenu() {
      this.$refs.menuRef.contentEl.focus();
    },
  },
});
</script>
